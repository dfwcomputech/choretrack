  ...
  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-slate-900/60 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        <h3 className="text-2xl font-bold text-slate-900">{t('children.removeChildAccount')}</h3>
        <p className="mt-3 text-sm text-slate-700">
          {t('children.removeConfirm', { name: childName || t('children.thisChild') })}
        </p>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          >
            {isDeleting ? t('children.removing') : t('children.removeChild')}
          </button>
        </div>
      </div>
    </div>
